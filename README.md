...
**Current release:** `0.16.0` · **Public beta · Governed Agent Teams**
**License:** MIT
...
- **Predictive routing in shadow mode** — historical recommendations are replayed offline, but version 0.16.0 still does not let the predictor autonomously change model tiers.
...
Version 0.16.0 still does **not** spawn a second independent LLM reviewer. Reviewer work is the human code-review gate; tester work is the governed validation contract on the same lease.
...
DynosAI `0.16.0` keeps the 0.14.x Control Plane/Studio and 0.15 harness authority model (schema v6) and adds governed team scheduling: file-disjoint leases, claim/fan-in, and reviewer/tester as contracts rather than extra spawned agents. The 0.13.0 core acceptance evidence remains the baseline for Codex/Cursor greenfield and brownfield behavior. The project is still pre-1.0 and should be adopted with normal engineering review, repository backups, and project-specific validation profiles.
