...
**Current release:** `1.0.0-rc.8` (`1.0.0rc8`) · **RC8 · certification pending**
**License:** MIT
...
- **Predictive routing in shadow mode** — historical recommendations are replayed offline, but `1.0.0-rc.8` still does not let the predictor autonomously change model tiers.
...
Version `1.0.0-rc.8` still does **not** spawn a second independent LLM reviewer. Reviewer work is the human code-review gate; tester work is the governed validation contract on the same lease. Eval intelligence does not auto-start a provider. Autonomous is an execution profile, not a skipped-gate mode. Additional IDE clients are not 1.0 live-certified.

## Quick start

Clone the public RC and install it locally:
...
| [1.0.0-rc.8 release notes](docs/RELEASE_1.0.0-rc.8.md) | Provider runtime portability: user-local workspaces, Codex temp-home abort, candidate identity gate |
...
| [1.0.0-rc.3 release notes](docs/RELEASE_1.0.0-rc.3.md) | Eval maturity: acceptance ZIP importer, governed-change cost aggregates, stable prompt prefix |
...
| [Code quality review](docs/CODE_QUALITY.md) | Current module/LOC metrics and hotspot policy |
| [ADR: PyPI name](docs/adr/0001-pypi-distribution-name.md) | Distribution `dynosai`, import `dynosai_flow` |
| [ADR: Agent Plugins](docs/adr/0002-agent-plugins.md) | Portable `plugin.json` + `extensions.com.dynosai`; no DynosAI Pack |
| [ADR: MCP protocol eras](docs/adr/0003-mcp-protocol-eras.md) | `2026-07-28` plus legacy `2025-11-25` / `2025-06-18` on stdio |
