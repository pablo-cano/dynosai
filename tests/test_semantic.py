...
class SemanticTests(unittest.TestCase):
    ...
    def test_neural_semantic_index_and_hybrid_retrieval(self):
        ...
        engine.db.execute(
            "INSERT INTO feature_rules(feature_id,rule) VALUES(?,?)",
            ("FEATURE-0001", "La entrada indica la cantidad de términos que debe generar la serie."),
        )
        engine.db.execute(
    ...
    def test_semantic_provenance_does_not_rewrite_last_changed_on_reverification(self):
        engine = DynosAI(self.temp, semantic_provider=TinySemanticProvider())
        engine.initialize("Provenance")
        now = "2026-08-12T00:00:00+00:00"
        first_commit = engine.git.head()
        engine.db.execute(
            "INSERT INTO features(id,work_id,title,summary,status,commit_hash,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
            ("FEATURE-P", "PROJECT", "Auth token", "Valida token de autenticación", "validated", first_commit, now, now),
        )
        engine.semantic.reindex_authoritative(first_commit)
        row1 = engine.db.one(
            "SELECT created_commit,last_changed_commit,last_verified_commit FROM semantic_documents WHERE entity_type='feature' AND entity_id='FEATURE-P'"
        )
        marker = self.temp / "README.extra"
        marker.write_text("metadata only\n")
        engine.git._git("add", "README.extra")
        engine.git._git("commit", "-m", "docs: metadata change")
        second_commit = engine.git.head()
        engine.semantic.reindex_authoritative(second_commit)
        row2 = engine.db.one(
            "SELECT created_commit,last_changed_commit,last_verified_commit,indexed_at_commit FROM semantic_documents WHERE entity_type='feature' AND entity_id='FEATURE-P'"
        )
        self.assertEqual(row2["created_commit"], row1["created_commit"])
        self.assertEqual(row2["last_changed_commit"], row1["last_changed_commit"])
        self.assertEqual(row2["last_verified_commit"], second_commit)
        self.assertEqual(row2["indexed_at_commit"], second_commit)
    ...
    def test_vector_rows_are_authoritative_and_rebuildable(self):
        ...
        # sqlite-vec is the preferred backend; NumPy remains the supported fallback.
        backend = result["semantic"]["backend"]
        self.assertIn(backend, {"sqlite-vec", "numpy"})
        self.assertEqual(backend, engine.db.vector_backend())
